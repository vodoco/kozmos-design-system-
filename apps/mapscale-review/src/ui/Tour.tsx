...
export const TOUR_STEPS: TourStep[] = [
  {
    ...
    body:
      "A new floor-plan arrives, MapScale maps it, Pointr's mapping team corrects it — then how much of the floor changed decides what happens next. This is that flow, running on the live map.",
  },
  {
    screen: "mapContent",
    target: "tree",
    ...
    body:
      "You never have to open a level to find out. Terminal 3 and B Gates shows one of each state at once.",
  },
  {
    screen: "mapContent",
    target: "level-tags",
    ...
    body:
      "Under 20% changed publishes itself. 20–50% waits for you, and publishes after 7 days if you don't act. Over 50% is rejected — that much change means the wrong file, not a remodel.",
  },
  {
    screen: "levelEditor",
    target: "floorplan",
    ...
    body:
      "The current file, and MapScale's status attached to it — because the job belongs to this floor-plan, not beside it.",
  },
  {
    screen: "levelEditor",
    target: "upload",
    title: "Try an upload",
    body:
      "Each press walks one outcome in turn: amber review, green auto-publish, red rejection, then an unreadable file. You can also drag a file straight onto the map.",
  },
  {
    screen: "review",
    target: "magnitude",
    ...
    body:
      "The number is the share of floor area, measured after the mapping team has corrected the AI — not a count of features.",
  },
  {
    screen: "review",
    target: "changelog",
    ...
    body:
      "Green new, blue updated, red removed, purple your own override. Your decision — confirm, flag, reject — never recolours a row; risk is a neutral ⚠, never amber.",
  },
  {
    screen: "review",
    target: "review-map",
    ...
    body:
      "Every row here is a real feature on this floor, highlighted beside it. Hover a row or a shape and they answer each other.",
  },
  {
    screen: "mapContent",
    target: "add-building",
    ...
    body:
      "Five steps: details, drop floor-plans to make levels, align them to one reference floor, place the building on the globe, then review MapScale's guesses on the screen you just saw.",
  },
  {
    target: "feedback-comment",
    title: "Now tell us what you think",
    body:
      "Press Comment and click anything on screen to pin a note to it. Everyone reviewing sees everyone's notes.",
  },
];
...
export function Tour({
  ...
  useLayoutEffect(() => {
    ...
    const timers = [80, 220, 500, 900].map((ms) => window.setTimeout(measure, ms));
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowRight") onStep(Math.min(step + 1, TOUR_STEPS.length - 1));
  ...
  if (hole) {
    ...
    const left = spaceRight > CARD_W + 24 ? hole.left + hole.width + 16 : Math.max(16, hole.left - CARD_W - 16);
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 70 }}>
      ...
      {hole ? (
        ...
        <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(12,14,20,.62)" }} />
      )}
      ...
      >
        ...
        <div style={{ padding: "14px 16px", fontSize: 13, lineHeight: 1.55, color: "#1a1c24" }}>{s.body}</div>
        <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 16px 14px" }}>
          <button
            onClick={onClose}
            style={{ border: "none", background: "none", color: "#737373", fontSize: 12, cursor: "pointer", padding: 0 }}
          >
            Skip tour
          </button>
          <span style={{ flex: 1 }} />
          {step > 0 && (
            <Button size="sm" variant="outline" onClick={() => onStep(step - 1)}>
              Back
            </Button>
          )}
          <Button
            size="sm"
            onClick={() => (step + 1 < TOUR_STEPS.length ? onStep(step + 1) : onClose())}
          >
