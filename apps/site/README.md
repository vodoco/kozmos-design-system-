...
1. **Register it** in `src/examples/manifest.ts`: `slug`, `title`, `kind`
   ...
   start); `featured: true` also shows it small on the home page. The route
   is created from this entry, and the search finds it.
