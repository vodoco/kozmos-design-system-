...
> (§11.3). Two PRs are open — #48, a check for Tailwind classes that compile to nothing, and #49,
> npm publish readiness — and they conflict textually with each other (§3). The next piece of work
...
| Thing            | State                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `origin/main`    | `f89b734` — every PR through #47 merged (`git log -1 origin/main`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| Open PRs         | **two.** #48, `components:classes:check`, with `main` merged in and its baseline at **62/40/27**. #49, npm publish readiness and `packages:install:check`, CI green on every job at `b7e3362`, the install check included. Both add a script straight after `tokens:raw:check` in `package.json` and a CI step straight after _Verify No New Raw Values_, so **the second to merge has a textual conflict: keep both lines**. Merged since the last handoff: #45 the three defects, #46 that handoff, #47 the map mode toggle                                                      |
| Branches         | `origin` carries `main`, `codex/compiled-classes-check` (#48), `codex/npm-readiness` (#49) and `codex/wayfinding-map-panel` — the last is MAP-595's prototype, parked (§9). Everything merged has been deleted                                                                                                                                                                                                                                                                                                                                                                     |
| Packages         | `@kozmos/react` `tokens` `icons` `vue` `product-contracts`, all `0.0.1`, never published — `npm view @kozmos/react` is a 404 and `git tag` lists nothing. #49 marks `vue` private, gives all five a `license` field and the four public ones a LICENSE file and README, and adds `packages:install:check` (§4.5)                                                                                                                                                                                                                                                                   |
...
| Gates            | the eleven in §7 green on 2026-09-17, plus `pnpm native:check` and the STATUS check. `tokens:raw:check` is a ratchet: 35 raw colours across 7 components, 7 raw radii across 6 — unchanged. Two more arrive with #48 and #49 (§7)                                                                                                                                                                                                                                                                                                                                                  |
...
**#49 closes most of this list.** What it does, and what stays open, measured on 2026-09-17:
...
The mechanics, once #49 merges: a changeset per package (`pnpm changeset`, "minor" for a first
...
9. **Merging #48 and #49.** Both need a go-ahead, and the second has the textual conflict in §3.
10. **The React stylesheet ships Tailwind's global reset** (§4.5) — keep and document it, scope it
    under a Kozmos root, or ship it as a separate file.
11. **`ThemeProvider` stores the theme under `vite-ui-theme`** — rename before a first publish.
12. **React's ink is `#000000`, native's `#17191C`.** The Tailwind config maps `foreground` to
...
Two more arrive with the open PRs, and both read a build, so build first:
