...
| The taxonomy's palette                           | `Semantics.Category.{Accent,Fill,OnFill}` in `packages/tokens/src/tokens*.json`; `pnpm tokens:build`, then `pnpm tokens:native:copy` (a hand copy of `KozmosColors*` missed `KozmosDesignTokens.kt` and `colors.xml`); `pnpm tokens:contrast:check` |
