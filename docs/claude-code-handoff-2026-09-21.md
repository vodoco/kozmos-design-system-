...
1. **The run in Figma** with build `314962f54832` (`docs/figma-drift-2026-09-21.md` §9): relaunch
   and read the build in the panel's header; Update, one at a time, the nineteen sets listed
   there, Button included though it was updated on `ed50a03a1912` (not Update All Core, which a new build restarts at Link); Build Surface QA; Audit
...
1. `cd /private/tmp/kozmos-browser-compat.uqPMBD && git status && git log --oneline -8` — expect a
   clean tree with `aa876ce` as the last change commit on `claude/pointr-browse-repairs`. If the worktree is gone, check it
