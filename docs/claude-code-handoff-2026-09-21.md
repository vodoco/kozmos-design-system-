...
- **Decisions still Olcay's, from the five:**
  1. **RoutingInputGroup's container radius** — React 32 (the `2xl` primitive, not a role; its
     FeedbackCard and SaveLocationCard too), SwiftUI and Compose `panel` (24), Figma `container`
     (20) at a 12 inset. Recommended: `panel` everywhere, the role of a panel over the map and what
     both natives draw.
  2. **React's island turns light in dark mode** (`bg-foreground`); Figma, SwiftUI and Compose are
     black in both themes. Recommended: React scopes the pill to the dark theme
     (`data-theme="dark"`, `bg-background text-foreground`), as the natives now do. Figma's fill
     binds `Colors/foreground/1000`, which reads #0B0D12 in the file where the code's light token
     is white — worth a look in the variables panel (the token lacks the variables scope for REST).
  3. **The native FeedbackCard comment box** is the standard outlined text area; React's is washed
     and borderless, as the routing fields. Recommended: a washed style on the native text field and
     area, shared by both.
  4. **The stepper's accent** is theme/500 in Figma and on the natives, `primary` (theme/600) in React.
  5. **Compose's Code Connect** still shows the light palette in five files (Box, CategoryField,
     CategoryTile, LocationPin, Pagination): their mapping values are class initialisers, which
     cannot read a composable; Pagination's example could. It changes only with a republish.
  - Flagged as separate tasks: a Compose glass card shows its elevation shadow through the tint (and
    iOS may); fills fixed across themes (theme/500) under inks that flip read about 3.7:1 in the dark
    on both natives.
...
1. **The run in Figma** with build `c35a625c8160` (`docs/figma-drift-2026-09-21.md` §9, the
