...
- **Open from the 22nd, found while building those** (none blocks the run):
  - **SwiftUI's island** draws the compact pill 36 high and as wide as its container, and the
    minimal circle at 48, where React and Compose — and now Figma — draw 240×44 and 56.
    Recommended: SwiftUI follows the others, unless 36 is meant to match the hardware's island.
  - **The browse grid's rows**: 8 apart in React, Compose and Figma, 12 in SwiftUI, and each
    platform's comment calls its own the prototype's. Measure the prototype, then align.
  - **The panel's rule under the search**: React's is `Border/Subtle`, SwiftUI's and Compose's
    `Colors/foreground/300`, much darker. Figma follows React.
  - **A taxonomy symbol swapped into a tile by hand arrives black**: the tile's tint is an
    override on the outline it replaced, keyed by that outline's layers. The panel lays the
    accent on again after its swap; a designer recolours by hand (both sets' descriptions say
    so). Tinting through a mask would survive any swap but rebuilds the tile and changes what
    the audit reads; not recommended now.
...
1. **The run in Figma** with build `6fdc2ffbc635` (`docs/figma-drift-2026-09-21.md` §9, the
...
1. `cd /private/tmp/kozmos-browser-compat.uqPMBD && git status && git log --oneline -8` — expect a
   clean tree with `b6830f4` as the last change commit on `claude/pointr-browse-repairs`. If the worktree is gone, check it
