...
1. **The run in Figma** with build `1001317b6546` (`docs/figma-drift-2026-09-21.md` §9): the
   repair of 21:36 is done and read back; relaunch and read the build in the panel's header;
   Update All Product / SDK; then Update TreeItem, TreeChildItem, TreeParentItem, Tree and
   Timeline one at a time (not Update All Core, which a new build restarts from Link); Build
   Surface QA; Audit Library, expected with no warning; then `pnpm figma:verify`, the REST
   read-back of the text bindings and the washes, and renders of DirectionStep and a Selected
   CategoryTile. If TreeItem's "variant n of 216" stops moving, Copy Log before anything else.
   Do not run Apply Text Styles, nor publish the library before the audit is clean. The §4 steps
   before it are done. Code Connect is published (drift §9); publish again only from this branch
   or from `main` after the merge, then `pnpm figma:connect:readback` with Figma in front. Still to
...
1. `cd /private/tmp/kozmos-browser-compat.uqPMBD && git status && git log --oneline -8` — expect a
   clean tree with `b1d7702` as the last change commit on `claude/pointr-browse-repairs`. If the worktree is gone, check it
