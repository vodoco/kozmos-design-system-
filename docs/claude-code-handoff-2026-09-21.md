...
1. **The run in Figma** with build `53ac76afe679` (`docs/figma-drift-2026-09-21.md` §9): relaunch
   and read the build in the panel's header; Update, one at a time, the seventeen sets listed
   there (Button and Badge were updated at 20:38 on `314962f54832`, which paints the same; not
   Update All Core, which a new build restarts at Link); Build Surface QA; Audit Library,
   expected with no warning and 53 advisories; then `pnpm figma:verify`, the REST read-back of
   the washes, and renders of DirectionStep and a Selected CategoryTile. The §4 steps before it
   are done. Code Connect is published (drift §9); publish again only from this branch or
...
1. `cd /private/tmp/kozmos-browser-compat.uqPMBD && git status && git log --oneline -8` — expect a
   clean tree with `5639895` as the last change commit on `claude/pointr-browse-repairs`. If the worktree is gone, check it
