...
1. **The repair and the run in Figma** with build `01f3be6891dc` (`docs/figma-drift-2026-09-21.md`
   §9): restore the version from before 21:36, when Apply Text Styles unbound 4,957 texts and set
   656 at 14/20; relaunch and read the build in the panel's header; Update, one at a time, the
...
1. `cd /private/tmp/kozmos-browser-compat.uqPMBD && git status && git log --oneline -8` — expect a
   clean tree with `bd4afde` as the last change commit on `claude/pointr-browse-repairs`. If the worktree is gone, check it
