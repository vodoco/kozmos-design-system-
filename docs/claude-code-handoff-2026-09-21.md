...
- **Open from the 22nd** (numbers in `docs/figma-drift-2026-09-21.md` §9, the last two
  sections): **BrowseCategoriesPanel's icons** — the aviation quick access's icons are the map
  sprite's type icons (`entrance-exit`, `service-space_office`, `security-space`,
  `transportation-space_boarding-gate`, `amenity-space_desk`, `parking-space`,
  `food-beverage-space`, `retail-space`; `get_quick_access`, 10.12.0), none of them among the
  56 on the Icons page, so every tile shows the tile's default bus. Recommended: add them to
  `packages/icons` and the Icons page, then set each tile's icon; a lookalike from the 56 would
  say what the product does not. **DynamicIsland's "•"**, its Minimal slot's placeholder, which
  `figma:verify` reports as an icon typed as a character: exempt slot placeholders from that
  check, or draw "Icon / Slot Default" there. **The panel's title**: Figma draws "Browse
  categories" as a 24 heading, while React's `label` is only the section's accessible name and
  draws nothing; keep it for the designer or drop it to match.
...
1. **The run in Figma** with build `7241e855b611` (`docs/figma-drift-2026-09-21.md` §9, the
   last section): relaunch and read the build in the panel's header; Update All Product / SDK
   (CategoryTile now runs before BrowseCategoriesPanel); Update Dialog, then Drawer, one at a
   time; Audit Library, expected with no warning and 54 advisories; then `pnpm figma:verify`
   (every enforced check ok, no overflow, one typed glyph: the "•" in §7) and the REST read-back
   of the panel's counts, its two-line names, DynamicIsland's slots and the two footers. The Tree
   block and the 66 Core sets need no Update: their builds draw as this one (drift §9). Then the
   library can be published. Do not run Apply Text Styles. Code Connect is published and read
   back on every platform (drift §9); publish again only from this branch or from `main` after
   the merge, then `pnpm figma:connect:readback` with Figma in front.
...
1. `cd /private/tmp/kozmos-browser-compat.uqPMBD && git status && git log --oneline -8` — expect a
   clean tree with `fc1adcc` as the last change commit on `claude/pointr-browse-repairs`. If the worktree is gone, check it
