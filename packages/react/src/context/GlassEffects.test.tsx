...
const TestController = ({ onUpdate }: { onUpdate?: (config: any) => void }) => {
    ...
    return (
        <div>
            <button data-testid="set-refraction-100" onClick={() => context.updateGlassConfig({ refraction: 100 })}>Refraction 100</button>
            <button data-testid="set-depth-100" onClick={() => context.updateGlassConfig({ depth: 100 })}>Depth 100</button>
            <button data-testid="set-surface-20" onClick={() => context.updateGlassConfig({ surfaceScale: 20 })}>Surface 20</button>
            <button data-testid="set-frost-50" onClick={() => context.updateGlassConfig({ frost: 50 })}>Frost 50</button>
            <button data-testid="set-specular-100" onClick={() => context.updateGlassConfig({ specular: 100 })}>Specular 100</button>
        </div>
    );
};
...
describe('Glass Engine Controls', () => {
    ...
    const renderWithProvider = (ui: React.ReactNode) => {
        // Passing null as persistKey forces a fresh default config every time
        return render(
            <DesignConfigProvider persistKey={null as any}>
                {ui}
    ...
    it('Surface Scale (Roughness) should control Surface Opacity', async () => {
        ...
        expect(root.style.getPropertyValue('--glass-surface-filter')).toBe('url(#glass-surface)');
    });
