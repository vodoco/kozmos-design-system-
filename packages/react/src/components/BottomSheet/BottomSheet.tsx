...
>(({ className, children, showClose = true, ...props }, ref) => (
  <DrawerContent
    ...
    showClose={false}
    className={cn(
      "inset-x-0 bottom-0 mt-24 flex h-auto max-h-[calc(100dvh-env(safe-area-inset-top)-1rem)] flex-col overscroll-contain rounded-t-lg border-t bg-background !px-4 !pb-[max(1.5rem,env(safe-area-inset-bottom))] !pt-0",
      className,
    )}
    {...props}
  >
    <div
      aria-hidden="true"
      className="mx-auto my-2 h-[5px] w-12 shrink-0 rounded-full bg-muted-foreground/40"
    />
    {children}
    {showClose ? (
      <BottomSheetClose className="absolute right-0 top-2 inline-flex h-11 w-11 items-start justify-end rounded-md p-2 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none">
