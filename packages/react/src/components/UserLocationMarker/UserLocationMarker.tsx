...
const UserLocationMarker = React.forwardRef<HTMLDivElement, UserLocationMarkerProps>(
  ({ className, heading = 0, showHeading = true, ...props }, ref) => {
    return (
      <div
        ref={ref}
        className={cn("relative flex items-center justify-center min-h-[48px] min-w-[48px]", className)}
        {...props}
      >
        ...
        {showHeading && (
           ...
           >
              ...
              <svg viewBox="0 0 100 100" className="h-full w-full">
                 <path d="M50 50 L85 10 A 50 50 0 0 0 15 10 Z" fill="url(#coneGradient)" opacity="0.4" />
                 <defs>
                   <radialGradient id="coneGradient" cx="50%" cy="50%" r="50%">
                      <stop offset="0%" stopColor="var(--semantics-data-blue)" stopOpacity="1" />
                      <stop offset="100%" stopColor="var(--semantics-data-blue)" stopOpacity="0" />
                   </radialGradient>
                 </defs>
              </svg>
           </div>
