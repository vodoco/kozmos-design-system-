...
const DynamicIsland = React.forwardRef<HTMLDivElement, DynamicIslandProps>(
    ({ className, islandState = 'compact', compactLeading, compactTrailing, expandedContent, minimalContent, ...props }, ref) => {
        ...
        return (
            <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[100]" ref={ref}>
                <motion.div
                    initial={false}
                    animate={{
                        width: islandState === 'expanded' ? 'calc(100vw - 32px)' : islandState === 'minimal' ? 56 : 240,
                        maxWidth: 360,
                        height: islandState === 'expanded' ? 160 : islandState === 'minimal' ? 56 : 44,
                ...
                >
                    <AnimatePresence mode="popLayout" initial={false}>
                        {islandState === 'compact' && (
                            ...
                            >
                                <div className="flex items-center select-none">{compactLeading}</div>
                                <div className="flex items-center select-none">{compactTrailing}</div>
                            </motion.div>
                        )}
