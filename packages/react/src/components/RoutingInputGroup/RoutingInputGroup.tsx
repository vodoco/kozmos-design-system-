...
const RoutingInputGroup = React.forwardRef<HTMLDivElement, RoutingInputGroupProps>(
    ({ className, points, onPointChange, onSwap, onAddPoint, onRemovePoint, ...props }, ref) => {
        ...
        return (
            <div ref={ref} className={cn('flex items-start gap-3 w-full bg-white/70 dark:bg-black/70 backdrop-blur-3xl p-4 rounded-[length:var(--primitives-radius-2xl)] shadow-2xl ring-1 ring-black/5 dark:ring-white/10 transition-all duration-300', className)} {...props}>
                {/* Timeline UI */}
                        ...
                        return (
                            <React.Fragment key={`timeline-${point.id}`}>
                                {isLast ? (
                                    ...
                                    <Circle className={cn("w-3.5 h-3.5 shrink-0 border-2 rounded-full", isFirst ? "text-primary border-primary bg-primary/20" : "text-muted-foreground border-current")} />
                                )}
                    ...
                    {points.map((point, index) => (
                        <div key={point.id} className="flex items-center gap-2">
                            <Input
                                ...
                                placeholder={point.placeholder || (index === 0 ? "Choose Starting Point" : "Choose Destination")}
                            ...
                            />
                            {points.length > 2 && index > 0 && index < points.length - 1 && onRemovePoint && (
                                <Button
                                    variant="ghost"
                                    size="icon"
                ...
                <div className="flex flex-col gap-2 shrink-0 justify-center">
                    ...
                    {onAddPoint && (
                        <Button
                            variant="ghost"
                            size="icon"
                            className={cn("w-10 h-10 shrink-0 text-muted-foreground", points.length === 2 ? "mt-5" : "mt-0")}
                            onClick={handleAdd}
                        >
