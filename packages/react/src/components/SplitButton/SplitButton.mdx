...
import { SplitButton } from "@kozmos-ds/react";
export function Example() {
  const [action, setAction] = useState("None");
  return (
    <>
      <SplitButton
        onMainClick={() => setAction("Publish requested")}
        menuItems={[
          {
            label: "Save draft",
            onClick: () => setAction("Save draft requested"),
          },
        ]}
      >
        Publish
      </SplitButton>
      <output>Demo action: {action}</output>
    </>
  );
}
    `}
    vue={`
<script setup>
import { KozmosSplitButton } from "@kozmos-ds/vue"
</script>
