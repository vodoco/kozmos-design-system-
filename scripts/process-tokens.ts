 ...
 * Usage: npx tsx scripts/process-tokens.ts
...
async function main() {
    ...
    if (!fs.existsSync(VARIABLES_FILE)) {
        ...
        console.error('👉 Please export your variables using a Figma Plugin (e.g. "Variables Import/Export") and save as variables.json');
        process.exit(1);
    }
    ...
    try {
        ...
        const generateHybrid = (baseKey: string, overrideKey: string, semanticKey: string = 'Semantic Tokens/idle') => {
            let base = rawData[baseKey] || {};
            let override = rawData[overrideKey] || {};
            let semantic = rawData[semanticKey] || {};

            // Deep merge logic: Start with Base
            let merged = { ...base, ...semantic };
        ...
        const lightTokens = generateHybrid('Primitive Tokens/Light', 'Primitive Tokens/Dark');
        fs.writeFileSync(path.join(OUTPUT_DIR, 'tokens-light.json'), JSON.stringify(lightTokens, null, 2));

        // 2. Generate Dark Mode
        // Base: Dark (Correct Theme) | Overrides: Light (Correct Background/Foreground)
        const darkTokens = generateHybrid('Primitive Tokens/Dark', 'Primitive Tokens/Light');
        fs.writeFileSync(path.join(OUTPUT_DIR, 'tokens-dark.json'), JSON.stringify(darkTokens, null, 2));
