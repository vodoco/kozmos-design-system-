...
function recommendFigmaScope(token) {
    ...
    if (type === 'color') return ['FRAME_FILL', 'SHAPE_FILL', 'TEXT_FILL', 'STROKE_COLOR'];
    if (lowerPath.includes('radius')) return ['CORNER_RADIUS'];
    if (lowerPath.includes('spacing') || lowerPath.includes('gap')) return ['GAP'];
    if (lowerPath.includes('font-size')) return ['FONT_SIZE'];
    if (lowerPath.includes('line-height')) return ['LINE_HEIGHT'];
    if (lowerPath.includes('width') || lowerPath.includes('height')) return ['WIDTH_HEIGHT'];
...
function inspectCodeConnectMapping(repoPath) {
    ...
    const placeholderChecks = [
        ...
        ['placeholder Figma URL', /https:\/\/(?:www\.)?figma\.com\/(?:file|design)\/XXXXX/i],
    ];
    const issues = placeholderChecks
        .filter(([, pattern]) => pattern.test(content))
        .map(([issue]) => issue);
    const hasConnectCall = /figma\.connect\(|@FigmaConnect|figmaNodeUrl|FigmaConnect\(/.test(content);
    ...
    function visit(node) {
        if (
            ...
            if (config && ts.isObjectLiteralExpression(config)) {
                const variantsProperty = config.properties.find((property) => (
                ...
                if (variantsProperty && ts.isPropertyAssignment(variantsProperty) && ts.isObjectLiteralExpression(variantsProperty.initializer)) {
                    ...
                    for (const property of variantsProperty.initializer.properties) {
                        if (ts.isPropertyAssignment(property) && ts.isObjectLiteralExpression(property.initializer)) {
                            ...
                            if (axisName) axes[axisName] = readObjectPropertyNames(property.initializer).sort((a, b) => a.localeCompare(b));
                        }
                    }
...
function collectVariantProps(typeArgument, context, props) {
    ...
    const variantSource = context.cvaVariants.get(typeArgument.exprName.getText(context.sourceFile));
...
function collectKnownExternalProps(typeNode, context, props) {
    return collectKnownExternalPropsFromText(typeNode.getText(context.sourceFile), props);
}

function collectKnownExternalPropsFromText(typeText, props) {
    if (!/(?:React\.)?ComponentProps(?:WithoutRef|WithRef)?</.test(typeText)) return false;

    if (/CheckboxPrimitive\.Root|SwitchPrimitive\.Root/.test(typeText)) {
        addProps(props, ['checked', 'children', 'defaultChecked', 'disabled', 'id', 'required', 'value']);
        return true;
    }

    if (/SliderPrimitive\.Root/.test(typeText)) {
        addProps(props, ['children', 'defaultValue', 'disabled', 'id', 'inverted', 'max', 'min', 'name', 'orientation', 'step', 'value']);
        return true;
    }
    ...
    if (/SelectPrimitive\.Root/.test(typeText)) {
        addProps(props, ['children', 'defaultOpen', 'defaultValue', 'disabled', 'name', 'open', 'required', 'value']);
        return true;
    }
    ...
    if (/RadioGroupPrimitive\.Item/.test(typeText)) {
        addProps(props, ['checked', 'children', 'disabled', 'id', 'required', 'value']);
        return true;
    }
...
function collectKnownImportedProps(localName, context, props) {
    ...
    addProps(props, ['asChild', 'children', 'disabled', 'isLoading', 'size', 'variant']);
    context.variants.size = ['default', 'icon', 'lg', 'sm'];
    context.variants.variant = ['default', 'destructive', 'ghost', 'glass', 'link', 'outline', 'secondary'];
    return true;
}
...
function collectPropsFromHeritage(heritageClauses, context, props, seen) {
    ...
    for (const clause of heritageClauses) {
        for (const type of clause.types) {
            ...
            if (collectKnownExternalPropsFromText(type.getText(context.sourceFile), props)) {
                continue;
            }
...
function collectPropsFromDefinition(definitionName, context, props, seen = new Set()) {
...
function collectPropsFromComponentDeclaration(sourceFile, componentName, context, props) {
    ts.forEachChild(sourceFile, function visit(node) {
        if (ts.isVariableDeclaration(node) && ts.isIdentifier(node.name) && node.name.text === componentName) {
            if (node.type && ts.isTypeReferenceNode(node.type)) {
                collectPropsFromTypeNode(node.type.typeArguments?.[0], context, props, new Set());
            }

            if (node.initializer && ts.isCallExpression(node.initializer)) {
                const callName = node.initializer.expression.getText(sourceFile);
                if (callName === 'React.forwardRef' || callName === 'forwardRef') {
                    collectPropsFromTypeNode(node.initializer.typeArguments?.[1], context, props, new Set());
                    const renderFunction = node.initializer.arguments[0];
                    if (renderFunction && (ts.isArrowFunction(renderFunction) || ts.isFunctionExpression(renderFunction))) {
                        collectPropsFromFunctionLike(renderFunction, context, props);
                    }
                }
            } else if (node.initializer && (ts.isArrowFunction(node.initializer) || ts.isFunctionExpression(node.initializer))) {
...
function addCommonInheritedProps(sourceFile, props) {
    ...
    if (/React\.(?:ButtonHTMLAttributes|InputHTMLAttributes|TextareaHTMLAttributes|AnchorHTMLAttributes|HTMLAttributes|ComponentProps)|ComponentPropsWithoutRef/.test(sourceText)) {
    ...
    if (/(?:InputHTMLAttributes|TextareaHTMLAttributes)/.test(sourceText)) {
        for (const propName of ['disabled', 'placeholder', 'readOnly', 'required', 'value']) {
...
function extractProps(repoPath, componentName) {
    ...
    const content = read(repoPath);
    const sourceFile = ts.createSourceFile(repoPath, content, ts.ScriptTarget.Latest, true, ts.ScriptKind.TSX);
    const context = {
        sourceFile,
    ...
    collectPropsFromComponentDeclaration(sourceFile, componentName, context, propNames);
    addCommonInheritedProps(sourceFile, propNames);

    return {
        props: [...propNames].sort((a, b) => a.localeCompare(b)),
        variants: Object.fromEntries(Object.entries(context.variants).sort(([a], [b]) => a.localeCompare(b))),
    };
}
...
function discoverReactComponents() {
    ...
    return fs.readdirSync(componentRoot)
        ...
        .map((name) => {
            ...
            const unmappedProps = codeConnect.mappedProps
                .filter((propName) => !componentApi.props.includes(propName));
...
function summarizeComponents(components) {
    return {
        ...
        codeConnectFiles: components.filter((component) => component.codeConnect.file).length,
        codeConnectScaffolds: components.filter((component) => component.codeConnect.status === 'scaffold').length,
        codeConnectLinked: components.filter((component) => component.codeConnect.linked).length,
    };
}
...
function buildManifest() {
    ...
    return {
        ...
        figma: {
            ...
            publishStatus: 'blocked until real component node IDs replace node-id=TBD',
        },
        tokens: {
            summary: {
