...
const DEFAULT_OUTPUT = path.join(
  ROOT_DIR,
  "docs/figma-pointr-icon-catalog.json",
);
...
async function main() {
  ...
  if (!token) {
    throw new Error(
      "Missing FIGMA_ACCESS_TOKEN. Add it to .env before running figma:icons.",
    );
  }

  const [nodeResponse, componentResponse] = await Promise.all([
    figmaRequest(
      `/files/${args.fileKey}/nodes?ids=${encodeURIComponent(args.rootNodeId)}&depth=3`,
      token,
    ),
  ...
  if (!rootNode) {
    throw new Error(
      `Could not find icon root node ${args.rootNodeId} in file ${args.fileKey}.`,
    );
  }

  const publishedComponents = componentResponse.meta?.components || [];
  const componentMetaByNodeId = new Map(
    publishedComponents.map((component) => [component.node_id, component]),
  );
  ...
  await writeGeneratedJson(args.output, catalog, {
    volatileKeys: ["generatedAt"],
  });

  console.log(
    `Wrote ${path.relative(ROOT_DIR, args.output)} with ${icons.length} icons across ${categories.length} categories.`,
  );
}
