 ...
 * Usage: FIGMA_ACCESS_TOKEN=x FIGMA_FILE_KEY=y pnpm exec tsx scripts/sync-figma.ts
 */
...
async function main() {
  ...
  try {
    ...
    if (meRes.ok) {
      ...
      console.warn(
        `⚠️ Token Identity Check Failed: ${meRes.status}. Your token might lack 'profile:read' scope.`,
      );
    }
    ...
    if (!basicRes.ok) {
      throw new Error(
        `File Access Failed: ${basicRes.status} ${basicRes.statusText}`,
      );
    }

    const fileData = await basicRes.json();
    console.log(
      `✅ File Found: "${fileData.name}" (Last Modified: ${fileData.lastModified})`,
    );
    ...
    if (fileData.role === "viewer") {
      console.warn(
        "⚠️ Warning: You are a Viewer on this file. Variables API usually requires Editor access in some implementations, but Viewer should work for READ.",
      );
    }
    ...
    if (!response.ok) {
      ...
      if (response.status === 403) {
        throw new Error(
          `Variables Access Failed (403). Ensure token has 'file_variables:read' scope.`,
        );
      }
      throw new Error(
    ...
    console.log(
      `✅ Found ${Object.keys(meta.variables).length} variables in ${Object.keys(meta.variableCollections).length} collections.`,
    );
    ...
    const resolveValue = (val: any): any => {
      if (val && val.type === "VARIABLE_ALIAS") {
        ...
        return `{${sanitizePath(targetVar.name).join(".")}}`;
      }
    ...
    Object.values(variables).forEach((v: any) => {
      ...
      const pathArray = sanitizePath(v.name);
      ...
      if (!lightVal)
        lightVal = Object.values(v.valuesByMode).map(resolveValue)[0];
      if (!darkVal) darkVal = lightVal;

      if (lightVal)
        setNested(tokensLight, pathArray, { $value: lightVal, $type: type });
      if (darkVal)
        setNested(tokensDark, pathArray, { $value: darkVal, $type: type });
    });

    fs.writeFileSync(
      path.join(TOKENS_DIR, "tokens-light.json"),
      JSON.stringify(tokensLight, null, 2),
    );
    fs.writeFileSync(
      path.join(TOKENS_DIR, "tokens-dark.json"),
      JSON.stringify(tokensDark, null, 2),
    );

    console.log(
      "✅ Generated tokens-light.json and tokens-dark.json directly from Figma RAW metadata seamlessly!",
    );
  } catch (error) {
